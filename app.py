...
app.config['MAIL_USERNAME'] = 'limihyle@gmail.com' # Apni Gmail ID
app.config['MAIL_PASSWORD'] = 'vqdiypfzmjabjaht'        # 16-digit App Password (bina space ke)
mail = Mail(app)
        ...
        "Physics Wallah": [
            {
                ...
                image: "https://via.placeholder.com/300x170?text=Lakshya+NEET",
                description: "Lakshya NEET 2027 — Class 12 NEET preparation with complete syllabus coverage and exam-focused practice.",
                classes: [
                    { name: "Physics", chapters: "Class 12 Complete", icon: "⚡", bg: "#eff6ff" },
                    { name: "Physical Chemistry", chapters: "Class 12 Complete", icon: "⚛️", bg: "#fef2f2" },
                    { name: "Organic Chemistry", chapters: "Class 12 Complete", icon: "🧪", bg: "#fdf2f8" },
                    { name: "Inorganic Chemistry", chapters: "Class 12 Complete", icon: "⚗️", bg: "#eff6ff" },
                    { name: "Botany", chapters: "Class 12 Complete", icon: "🌱", bg: "#fdf2f8" },
                    { name: "Zoology", chapters: "Class 12 Complete", icon: "🧬", bg: "#eff6ff" },
                    { name: "DPP & Tests", chapters: "Practice + Tests", icon: "📝", bg: "#fefce8" }
                ],
    ...
    activeBatches.forEach((batch, index) => {
        ...
        container.innerHTML += `
            <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:16px; padding:12px; margin-bottom:16px; box-shadow:0 2px 8px rgba(0,0,0,0.05);">
                ...
                <div style="font-size:18px; font-weight:800; color:#171717; padding:14px 2px 12px;">
                    ${batch.title}
                </div>
...
function renderBatchSubjects() {
    ...
    classes.forEach((sub) => {
        ...
        container.innerHTML += `
            <div onclick="openSubjectLectures('${safeName}')"
...
function renderSubjectLectures() {
    ...
    sampleLecturesData.forEach((lec) => {
        let videoId = "";
        if (lec.youtubeUrl) {
...
def send_otp():
    ...
    try:
        msg = Message('X STUDY Verification OTP', sender=app.config['MAIL_USERNAME'], recipients=[identifier])
        msg.body = f'Your X STUDY App OTP Code is: {otp}\n\nValid for 20 minutes. Do not share this code with anyone.'
        mail.send(msg)
