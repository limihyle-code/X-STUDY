...
function openSubjectLectures(subjectName, classIndex) {
    const classes = (currentBatchDetails && Array.isArray(currentBatchDetails.classes))
        ? currentBatchDetails.classes : [];
    const selectedClass = (Number.isInteger(classIndex) ? classes[classIndex] : null)
        || classes.find(c => c.name === subjectName)
        || { name: subjectName || 'Class' };

    // Physics Wallah: original class flow — directly open green LECTURES/NOTES/PNG/DPP page.
    if (currentInstitute === 'Physics Wallah') {
        currentSubjectDetails = selectedClass;
        currentTeacherClass = selectedClass;
        currentTeacherItems = [];
        document.getElementById('batch-details-screen').style.display = 'none';
        document.getElementById('teacher-page-screen').style.display = 'none';
        document.getElementById('video-batch-title').innerText = selectedClass.name || subjectName || 'Class';
        document.getElementById('video-screen').style.display = 'flex';
    ...
    // Career Will: screenshot-style class page -> clickable content item -> green page.
    currentTeacherClass = selectedClass;
    ...
    document.getElementById('teacher-page-title').innerText = currentTeacherClass.name || subjectName || 'Class';
...
function closeVideoScreen() {
    ...
    document.getElementById('video-screen').style.display = 'none';
    if (currentInstitute === 'Career Will') {
        document.getElementById('teacher-page-screen').style.display = 'flex';
        renderTeacherPageItems();
    } else {
        document.getElementById('batch-details-screen').style.display = 'flex';
